...
namespace Solaire{ namespace Maths{
	...
	*/
	...
	public:
		...
		*/
		constexpr Vector3(const value_t aX, const value_t aY, const value_t aZ) :
			X(aX),
			Y(aY),
			Z(aZ)
		{
		...
		*/
		constexpr bool operator==(const Vector3<value_t> aOther) const{
		...
		*/
		 constexpr Vector3<value_t> operator+(const Vector3<value_t> aOther) const{
		...
		/*!
			...
			\return The result of the operation.
		*/
		 constexpr Vector3<value_t> operator+(const value_t aScalar) const{
			return Vector3<value_t>(
                X + aScalar,
                Y + aScalar,
                Z + aScalar
		...
		*/
		constexpr value_t DotProduct(const Vector3<value_t> aOther) const{
		...
		*/
		 constexpr Vector3<value_t> CrossProduct(const Vector3<value_t> aOther) const{
			return Vector3<value_t>(
		...
		*/
		 constexpr Vector3<value_t> Normalise() const{
	...
	*/
	...
	static constexpr Vector3<A> vector_cast(const Vector3<B> aVector){
		return Vector3<A>(
